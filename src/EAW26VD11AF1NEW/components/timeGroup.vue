  ...
  <div>
    ...
    <mt-popup class="popup-timing" v-model="show" position="bottom">
      ...
      <div class="bd" v-if="show">
        <div :style="{overflow: 'hidden', position: 'relative', height: height}">
          <scroller>
            <div v-for="it in list" :key="it.taskId">
              <div class="bd-list J_fingler_handler" :id="it.taskId" :class="{'bd-list-swiper': it.swiper}">
                <div class="bdl">
                  <p>{{showInlineDesc(it)}}</p>
                  <span>{{showTextByPeriod(it.period)}}</span>
                </div>
                <div class="bdr">
                  <mt-switch v-model="it.enable"></mt-switch>
                </div>
                <div class="btn btn-handler"><span class="btn-handler">删除</span></div>
              </div>
            </div>
          </scroller>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      taskId: null,
      list: [],
      // list: [{ taskId: '1', action: JSON.stringify({ as: { 0: 0 } }), period: 2, time: 0, swiper: false, enable: false }, { taskId: '2', action: JSON.stringify({ as: { 0: 0 } }), period: 2, time: 0, swiper: false, enable: false }, { taskId: '3', action: JSON.stringify({ as: { 0: 0 } }), period: 2, time: 0, swiper: false, enable: false }],
      xVisible: false,
      checklist: [],
      checklist1: [],
      times: [
  ...
  methods: {
    tap (e, id) {
      ...
      if (e.target.className === 'mint-switch') {
        this.toggleEnable(list)
        return false
      }
      if (e.target.className.indexOf('btn-handler') > -1) {
        this.removeTimer(id)
        return false
      }
      this.editTimeHandler(list)
    },
    swipe (e, id) {
      console.log(e)
      this.list.map(it => {
        it.swiper = (it.taskId === id && e.direction === 'Left')
        return it
      })
    },
    ...
    saveTimeHandler () {
      ...
      opts.time = 0;
      console.log(this.minuteValue,'this.minuteValue----',this.minuteValue.split(':'));
      ...
      Array.from(this.list).forEach((item)=>{
        if (item.time==opts.time&&opts.type===2){
          isAdd=true
        }
      });
      if (isAdd) {
        return this.$toast('此时间已有定时任务');
      }
      opts.enable = true
      if (this.taskId) {
    ...
    addTimer (opts) {
      ...
      console.log(opts,'addTimer opts-------');
    ...
    setHeight () {
      var els = document.getElementsByClassName('J_fingler_handler')
      let height = 0
    ...
    bindFinger () {
      ...
      for (var i = 0; i < els.length; i++) {
        var el = els[i]
        const al = new window.AlloyFinger(el, {
          singleTap: function (e) {
            that.tap(e, this.id)
          },
          swipe: function (evt) {
            that.swipe(evt, this.id)
          }
        })
        this.fingers.push(al)
      }
    }
  }
}

...
export default {
  ...
  computed: {
    showTempText () {
      const { other } = this
      return other.mold === '4' ? other.indoorTemperature : other.temperature
    },
    canAdjust () {
      return ['0', '2', '4'].indexOf(String(this.other.mold)) === -1 && this.other.onoff && this.other.online
    }
  },
  methods: {
    // 调节温度
  ...
  mounted () {
    ...
    this.dragGuage = new DragGuage(document.getElementById('echartContainer'), {
      noDrag () {
        ...
        if (!_this.other.mold || ['0', '2', '4'].indexOf(String(_this.other.mold)) > -1) {
          return true
        }
      },
      touchStart () {

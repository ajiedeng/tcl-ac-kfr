<template>
  <div class="picker-slot" :class="classNames" :style="flexStyle">
    <div v-if="!divider" ref="wrapper" class="picker-slot-wrapper" :class="{ dragging: dragging }" :style="{ height: contentHeight + 'px' }">
      <div class="picker-item" v-for="(itemValue, i) in mutatingValues" :key="i" :class="{ 'picker-selected': itemValue === currentValue }" :style="{ height: itemHeight + 'px', lineHeight: itemHeight + 'px' }">
...
export default {
  ...
  methods: {
    ...
    translate2Value (translate) {
      ...
      var index = Math.ceil(Math.abs(-(translate - Math.floor(this.visibleItemCount / 2) * itemHeight) / itemHeight).toFixed(4))

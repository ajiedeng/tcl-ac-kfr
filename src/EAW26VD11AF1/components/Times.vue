          ...
          <ul class="myul">
            ...
            >
              <div class="timcont flex">
                <div  style="width:100%;"   @click="modifytim(item.id,item.istimon,item.typ,item.ifRepeat,item.weekdayinfo,item.hour,item.min)"   >
                  <div   v-if="item.hour <= 9 && item.min <= 9"
                    class="timdiv"   >0{{item.hour}} : 0{{item.min}} {{item.typ}}</div>
                  <div  v-else-if="item.hour <= 9"  class="timdiv">0{{item.hour}} : {{item.min}} {{item.typ}}</div>
                  <div  v-else-if="item.min <= 9"  class="timdiv">{{item.hour}} : 0{{item.min}} {{item.typ}}</div>
                  <div v-else class="timdiv">{{item.hour}} : {{item.min}} {{item.typ}}</div>
                  <div style="color:rgba(153,153,153,1);font-size:0.4rem">{{item.weekday}}</div>
                </div>
                <el-switch
                  class="myswicth"
    ...
    <div id="timing" v-if="selectTimeView">
      <div class="box">
        ...
        <div class="header flex">
          <div class="myleft flex" data-val="2.1" @click="closeTimeSelePageFn">取消</div>
      ...
      timlist:[          // 定时列表
        {
          id: 111,
          realid: 0,
          istimon: true, // 这个定时是否开启
          typ: "关闭设备",
          ifRepeat: false,
          weekday: "不重复",
          weekdayinfo: [0,0,0,0,0,0,0],
          hour: 19,
          min: 19,
        }
      ],
  ...
  methods: {
    ...
    getTimersFn() {
      ...
      device.getTimers(function(res, datas) {
        ...
        // }]
        // let res = null;
        // alert('【haveTimes data >>>>>】' + JSON.stringify(datas));
        if (res === null) {
